...
def map_messages(row):
    ...
    txt=row['question']
    row['messages'] = [{'role':'system','content':'You are a language model that answers the given question. The answer must be a single term that can be the answer of the question, without auxiliary terms.'},{'role':'user','content':txt}]
    # print(row['messages'])
    # assert()
    return row
...
if __name__=="__main__":
    ...
    parser.add_argument('--dataset_path', type=str, default='data/entityrag/musique_result_or2f.jsonl')
    parser.add_argument('--model_path', type=str, default='meta-llama/Llama-3.1-8B-Instruct', help='Specific model name')
    parser.add_argument("--save_path", type=str, default="data/musique_result_or2f_gen.jsonl", help="path to inference data to evaluate (e.g. inference/baseline/zero_v1/Llama-3.1-8B-Instruct)")
